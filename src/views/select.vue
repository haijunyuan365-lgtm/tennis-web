        ...
        <div class="court-panel">
          <div class="court-container">
            <div class="court-background-container">
              <!-- 背景图容器 -->
              <img class="court-background" src="../assets/image/test1.jpg" alt="场地背景" />
              <!-- 添加标题文字 -->
        ...
        <div class="players-panel">
          ...
          <div class="select-container">
            ...
            <div class="players-table">
              <el-table :data="players" style="width: 100%" class="players-container"
                ...
                <el-table-column width="250" align="right" label="操作">
                  <template v-slot:default="{ row }">
                    <el-radio-group :value="row.selected" size="medium">
                      <el-radio-button label="A" border
                        @click.native.prevent="toggleRegion(row, 'A', $event)">区域A</el-radio-button>
                      <el-radio-button label="B" border
                        @click.native.prevent="toggleRegion(row, 'B', $event)">区域B</el-radio-button>
                    </el-radio-group>
                  </template>
                </el-table-column>
              </el-table>
            ...
            <div class="confirm-btn">
              <el-button type="primary" round :disabled="!isOpen" @click="handleConfirm">
                确认
              </el-button>
            </div>
          </div>
          ...
          <div class="action-footer">
            <el-button type="info" icon="el-icon-notebook-2" @click="showHistory">历史记录</el-button>
  ...
  methods: {
    ...
    validateRegionSelection(row, region) {
      ...
      if (this.gameMode === "single") {
        ...
        if (selectedPlayers.length >= 4) {
          this.$message.warning("单打模式最多只能选择4名球员(A区和B区各2名)");
          return false;
        }
        // 获取选择相同区域的球员数组
    ...
    handleConfirm() {
      ...
      if (this.canConfirm) {
        ...
        if (this.gameMode === 'single') {
          if (selectedPlayers.length < 2) {
            this.$message.error(`还需${2 - selectedPlayers.length}人:A区还需${1 - playersInRegionA.length}人, B区还需${1 - playersInRegionB.length}人`)
          }
        } else if (this.gameMode === 'double') {
          if (selectedPlayers.length < 4) {
            this.$message.error(`还需${4 - selectedPlayers.length}人:A区还需${2 - playersInRegionA.length}人, B区还需${2 - playersInRegionB.length}人`)
          }
        }
      }
    },
...
/* 关键修改：使用背景色替代边框 */
.players-container ::v-deep .el-table__row {
...
.players-container ::v-deep .el-table__row.selected {
...
.players-container ::v-deep .el-table__row.selected+.el-table__row:not(.selected) {
  margin-top: 4px;
}

/* 移除边框样式 */
.players-container ::v-deep .el-table__row.selected td {
